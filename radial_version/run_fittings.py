#This script should be run in the terminal. Call the script with a single command line argument, which is the parameter text file you have used. 
###%%%###%%%###%%%###%%%###%%%###
...
def main(paramfile, fitType, ranges, guesses, productname):
    ...
    #Need to read the contents of the range and guess files. This looks funky but the text files were a pain so this is just the price I have to pay. 
    ...
    with Pool() as pool:
        sampler=es(paramdict['numWalkers'], ndim, lnpostfn, pool=pool, #note here that if you want a speed-up, eliminate the args passed in here (or at least make it such that the large arrays such as those in data are not passed. Memmap?)
                   args=[ranges, fitType, paramdict['radiusStart'], paramdict['radiusStep'], paramdict['radiusNumSteps'], data[0], data[1], data[2], data[3], data[4], data[5], data[6]])
        ...
        fit_start=time.time()
        pos1, prob1, state1 = sampler.run_mcmc(pos0, paramdict['runTime'])
        fit_end=time.time()
